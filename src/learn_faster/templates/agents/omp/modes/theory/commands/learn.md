...
1. **Gather learning preferences** with `ask` based on users selected topic:
   <example>
...
After each concept, invoke the `practice-creator` agent with the `task` tool to generate:
