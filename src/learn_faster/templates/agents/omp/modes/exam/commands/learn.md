...
1. **Gather exam preferences** with `ask` based on users selected topic:
   <example>
...
2. **Search online first** for real exam syllabus and structure:
    ...
    - Use `web_search` to find: "[topic] exam syllabus", "[certification] exam blueprint", "[subject] exam topics"
    - Use `read` to analyze official exam guides, topic breakdowns, and weightings
